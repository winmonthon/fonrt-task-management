...
<div class="px-5">
  <div class="mt-5  ">
    ...
    <div>
      <button class="w-full mb-5 px-4 py-2 tracking-wide text-white transition-colors duration-200 transform bg-blue-500 rounded-md hover:bg-blue-400 focus:outline-none focus:bg-blue-400 focus:ring focus:ring-blue-300 focus:ring-opacity-50" @click="$router.push(`/calendar/${userId}`)">
        Calendar
      </button>
    </div>
    ...
    <div v-for="(task, i) in tasks" :key="i" class="mobile-table">
      ...
      <div class="flex justify-between my-1">
        ...
        <div>
          <button type="button" @click="updateTask(i)" class="px-2 py-1 text-sm tracking-wide text-white transition-colors duration-200 transform bg-blue-500 rounded-md hover:bg-blue-400 focus:outline-none focus:bg-blue-400 focus:ring focus:ring-blue-300 focus:ring-opacity-50">
            Update
          </button>
          <button type="button" @click="finishTask(i)" class="px-2 py-1 text-sm tracking-wide text-white transition-colors duration-200 transform bg-green-500 rounded-md hover:bg-green-400 focus:outline-none focus:bg-green-400 focus:ring focus:ring-green-300 focus:ring-opacity-50">
            Finish
          </button>
        </div>
      </div>
    ...
    <div class="my-5 flex justify-center" role="group" aria-label="Second group">
      <div>
        <button type="button" @click="changePage(i)" class="border py-1 px-3 bg-gray-600 hover:bg-gray-800 rounded-sm" v-for="(page, i) in allPages" :key="i">
          {{ i + 1 }}
        </button>
      </div>
    </div>
...
export default {
  ...
  methods: {
    async getTasks() {
      const engineerData = await axios.get(
        `${process.env.VUE_APP_API}/users/check/lineuid/${this.lineUid}`
      );

      const userId = engineerData.data.data.userId;
      const tasks = await axios.get(
        `${process.env.VUE_APP_API}/task/engineer/${userId}?q=${this.searhText}&status=${this.statusFilter}`
      );
      console.log(tasks);
    ...
    async changePage(index) {
      const data = await axios.get(
        `${process.env.VUE_APP_API}/task/engineer/${this.userId}?page=${index +
          1}&size=10&q=${this.searhText}&status=${this.statusFilter}`
      );
